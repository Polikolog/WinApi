...
BOOL HT::CheckCurrentUser(HTHANDLE* ht, const wchar_t UserName[512])
{
	...
	if (UserName == NULL)
	{
		...
		for (int i{}; i < uc3; i++)
		{
			if (ht->HTUsersGroup == std::wstring(buf3[i].grui0_name) or L"Администраторы" == std::wstring(buf3[i].grui0_name))
			{
